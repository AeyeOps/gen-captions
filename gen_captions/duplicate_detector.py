    ...
    def scan_images(self) -> List[Path]:
        ...
        for ext in self.IMAGE_EXTENSIONS:
            self.image_files.extend(
                self.directory.glob(f"*{ext}")
            )
            self.image_files.extend(
                self.directory.glob(f"*{ext.upper()}")
            )
    ...
    def find_perceptual_duplicates(
        ...
        if not self._perceptual_hashes:
            iterator = (
                track(
                    self.image_files,
                    description="Computing perceptual hashes",
                    console=self.console,
                )
                if self.console
                else self.image_files
        ...
        for i, file1 in enumerate(files):
            ...
            if len(group) > 1:
                ...
                group_info = [
                    self._get_file_info(f) for f in group
                ]
