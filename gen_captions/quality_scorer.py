...
class QualityScorer:
    ...
    BAD_FILENAME_PATTERNS = ['copy', 'duplicate', 'temp', 'thumb', 'backup', '(1)', '(2)']
    ...
    def score_file(self, file_info: dict) -> float:
        ...
        if not any(bad in filename_lower for bad in self.BAD_FILENAME_PATTERNS):
            score += 10
    ...
    def recommend_keeper(self, group: List[dict]) -> Tuple[dict, str]:
        ...
        # Score each file
        scored = [(file_info, self.score_file(file_info)) for file_info in group]
        ...
        if best_file['width'] * best_file['height'] == max(f['width'] * f['height'] for f in group):
