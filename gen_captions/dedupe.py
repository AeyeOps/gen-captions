...
class DedupeProcessor:
    ...
    def __init__(self, directory: str, yolo_mode: bool = False, console: Console = None):
        ...
        self.detector = DuplicateDetector(str(directory), console=console)
    ...
    def run(self):
        """Main execution flow."""
        mode_str = '[YOLO MODE]' if self.yolo_mode else '[INTERACTIVE]'
        ...
        for name, desc, confidence, threshold in self.layers:
            skip_layer = self.process_layer(name, desc, confidence, threshold)
            if skip_layer:
                break
    ...
    def process_layer(self, name: str, desc: str, confidence: int, threshold) -> bool:
        ...
        """
        width = self.console.width if hasattr(self.console, 'width') else 70
        self.console.print('─' * width, style="dim")
        self.console.print(f"Layer: {name} ({confidence}% confidence match)")

        # Explain what this layer does
        explanations = {
            'EXACT': 'Finds files that are 100% identical (same bytes). These are safe to remove.',
            'IDENTICAL': 'Finds images that look identical but may have different metadata. Very safe.',
            'NEAR-IDENTICAL': 'Finds images with tiny differences (small crops, watermarks). Low risk.',
            'VERY_SIMILAR': 'Finds same image with different processing (filters, resolution). Medium risk.',
            'SIMILAR': 'Finds related images (burst photos, similar shots). Higher risk - review carefully.'
        }
        ...
        self.console.print(f"What this finds: {explanations.get(name, desc)}")
        self.console.print(f"Risk level: {risk_levels.get(name, 'UNKNOWN')}\n")
        ...
        if threshold == 'exact':
            ...
            groups = self.detector.find_perceptual_duplicates(threshold)
        ...
        self.console.print(f"Found {len(groups)} duplicate groups ({files_to_move} files to move)\n")
    ...
    def _process_layer_interactive(self, groups: list, layer_name: str) -> bool:
        ...
        for group in groups:
            ...
            duplicates = [f['name'] for f in group if f != keep_file]
            table.add_row(keep_file['name'], ", ".join(duplicates))
        ...
        self.console.print("      (s)kip - Skip this entire layer")
        self.console.print("      e(x)it - Stop and show summary", markup=False, highlight=False)
        ...
        if choice == 'c':
            ...
            self.console.print("      Invalid choice, skipping layer\n")
            return False

    def _process_all_groups(self, groups: list, layer_name: str) -> bool:
        """Process all groups in a layer."""
        for group in track(groups, description=f"Processing {layer_name}"):
            keep_file, reason = self.scorer.recommend_keeper(group)
            duplicates = [f for f in group if f != keep_file]
            self.execute_move(keep_file, duplicates, layer_name)

        # Layer summary
        if self.stats['by_layer'][layer_name] > 0:
            self.console.print(
                f"Layer complete: moved {self.stats['by_layer'][layer_name]} duplicates\n"
            )

        return False

    def process_group(self, group: list, group_num: int, total: int, layer_name: str, auto_mode: bool):
        ...
        """
        self.console.print(f"  [{group_num}/{total}] {len(group)} files are duplicates")

        # Add user-friendly explanation
        self.console.print(f"\n      These files are the same image. We'll keep one and move the")
        self.console.print(f"      others to duplicates/ folder so you can review or delete them later.")

        # Show files
        self.console.print()
        for file_info in sorted(group, key=lambda x: x['size'], reverse=True):
            caption_marker = '[HAS CAPTION]' if self.scorer.has_caption_file(Path(file_info['path'])) else ''
        ...
        self.console.print(f"      This will move {len(duplicates)} duplicate(s) to duplicates/ folder.")
        ...
        if self.yolo_mode or auto_mode:
            ...
            return self.prompt_user(keep_file, duplicates, group, layer_name)

    def prompt_user(self, keep_file: dict, duplicates: list, group: list, layer_name: str) -> str:
        ...
        if choice == '' or choice == 'c':
            ...
            self.console.print("      Invalid choice, continuing...\n")
            return 'continue'

    def execute_move(self, keep_file: dict, duplicates: list, layer_name: str):
        ...
        for dup in duplicates:
            if self.file_ops.move_to_duplicates(dup):
                self.console.print(f"      -> {dup['name']} moved to duplicates/")
                self.stats['moved'] += 1
                self.stats['space_saved'] += dup['size']
                self.stats['by_layer'][layer_name] += 1
    ...
    def show_summary(self):
        ...
        self.console.print(f"  Files kept:        {self.stats['kept']}")
        self.console.print(f"  Files moved:       {self.stats['moved']}")
        space_str = self.scorer.format_size(self.stats['space_saved'])
        ...
        if self.stats['by_layer']:
            ...
            for layer, count in self.stats['by_layer'].items():
                if count > 0:
                    self.console.print(f"    {layer:<15} {count} duplicates")

        self.console.print(f"\n  Deduped:    {self.directory}/")
        self.console.print(f"  Duplicates: {self.file_ops.duplicates_dir}/")
        self.console.print()
...
def dedupe_command(
    ...
    """
    processor = DedupeProcessor(image_dir, yolo_mode=yolo, console=console)
    processor.run()
