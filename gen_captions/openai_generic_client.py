...
class OpenAIGenericClient:
    ...
    def generate_description(self, image_path: str) -> str:
            ...
            try:
                ...
                if (
                    ...
                    if "[trigger]" not in description:
                        ...
                        self._logger.info(
                            r"Description missing \[trigger] for %s. Retrying...",
                            image_path,
                        )
                        self._console.print(
                            rf"[bold yellow]No \[trigger] token in response for {image_path}, retrying...[/]"
                        )
                    ...
                    self._console.print(
                        f"[bold green]Generated description for:[/] [italic]{image_path}[/]"
                    )
                    return description

                # If there's no 'choices' or no content, just treat as no content
                self._console.print(
                    f"[yellow]No content returned by LLM for:[/] {image_path}"
                )
                return ""
                ...
                code = 0
                if isinstance(re, APIConnectionError) and re.code:
                    code = int(re.code)
                ...
                if code == 429:
                    ...
                    self._console.print(
                        f"[bold yellow]Rate limit for {image_path}, retrying in "
                    ...
                    self._logger.error(
                        "API or HTTP error for %s: %s", image_path, re
                    )
                    self._console.print(
        ...
        self._console.print(
            f"[bold red]Failed to generate description after {retries} retries "
            f"for {image_path}[/]"
        )
        return ""
    ...
    def _build_chat_request(self, base64_image: str) -> dict:
        ...
        model_quirks = MODEL_CONFIG.get(model_name, None)
...
"""
        ...
        supports_system = model_quirks.get("supports_system_role", True)
        supports_temp = model_quirks.get("supports_temperature", True)
        max_tokens_key = model_quirks.get(
            "max_tokens_key", "max_completion_tokens"
        )
        max_tokens_value = model_quirks.get("max_tokens_value", 200)
        ...
        request_params[max_tokens_key] = max_tokens_value
